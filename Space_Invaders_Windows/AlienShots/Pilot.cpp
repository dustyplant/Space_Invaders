...
Pilot::Pilot( int X, int Y, SDL_Surface* image)
{
...
void Pilot::pilotMovement(std::vector<Shot> &hyperBeam, SDL_Surface* shots, clock_t &shotPause, int SCREEN_WIDTH){
    ...
    if(keystates[SDLK_RIGHT] && x + image->w < SCREEN_WIDTH){
        x += 10;
        shift_boxes();
    }
    if(keystates[SDLK_LEFT] && pilot.posx > 0){
        x -= 10;
        shitf_boxes();
    }
    if(keystates[SDLK_SPACE] && (float(clock()) - float(shotPause))/CLOCKS_PER_SEC*10 > .1){
