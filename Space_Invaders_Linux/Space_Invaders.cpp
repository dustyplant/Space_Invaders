...
int main(int argc, char* args[]){
	...
	do{
		...
		while(!quit && !dead){
			...
			if( int(((float(clock()) - float(survivalTime))/CLOCKS_PER_SEC))*10 % speedUp == 0 && int(((float(clock()) - float(survivalTime))/CLOCKS_PER_SEC)) != 0){
				if(frames_per > 0){
			...
			streamer << "Speed: " << varSpeed << "     Survival Time: " << ((float(clock()) - float(survivalTime))/CLOCKS_PER_SEC)*10;
